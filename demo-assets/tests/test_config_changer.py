...
def test_can_get_named_jobs_simple_entry(default_config_changer):
    name = 'python-test'
    job = default_config_changer.get_workflow_job_with_prefix(name)
...
def test_can_get_named_jobs_entry_with_info_and_name(default_config_changer):
    name = 'Deploy Dev'
    job = default_config_changer.get_workflow_job_with_prefix(name)
    assert(job['name'].startswith(name) )
...
def test_can_modify_context(default_config_changer):
    ...
    assert(job['name'].startswith('Deploy Dev')  )
    newjob = default_config_changer.get_workflow_job_with_prefix("Deploy Dev")
